...
function Initials({ name }: { name: string }) {
  ...
  return (
    <div className="w-full h-full bg-[#CC0000] flex items-center justify-center">
  ...
  return (
    <div className="bg-white border border-[#E5E5E5] rounded-2xl overflow-hidden flex flex-col">
      ...
      <div className="relative w-full h-[140px] bg-[#F9F9F9]">
        ...
        {item.featured && (
          <span className="absolute top-2 left-2 bg-[#CC0000] text-white text-xs font-bold px-2 py-0.5 rounded-full">
            ★ Popular
          </span>
        )}
      </div>
      ...
      <div className="p-4 flex flex-col gap-3 flex-1">
        <div className="flex justify-between items-start gap-2">
          ...
          <span className="text-[18px] font-bold text-[#CC0000] whitespace-nowrap">
            RM {price.toFixed(2)}
          </span>
        </div>
        ...
        {variants.length > 1 && (
          <div>
            ...
            <select
              ...
              className="w-full border border-[#E5E5E5] rounded-xl px-3 py-2 text-[16px] focus:outline-none focus:border-[#CC0000] bg-white min-h-[44px]"
            >
              {variants.map((v, idx) => (
