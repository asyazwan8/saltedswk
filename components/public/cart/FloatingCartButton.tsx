...
export default function FloatingCartButton({ onOpen }: { onOpen: () => void }) {
  ...
  return (
    <div className="fixed bottom-6 left-0 right-0 z-30 flex justify-center px-4 pointer-events-none">
      <button
        onClick={onOpen}
        className="pointer-events-auto flex items-center gap-3 bg-[#1A1A1A] hover:bg-[#000000] text-white font-bold text-[16px] px-6 min-h-[56px] rounded-2xl shadow-2xl transition-colors"
